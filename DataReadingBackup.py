...
from SiigmaticReportAutomation.GPTAnalysis import *
...
db = openpyxl.load_workbook(file, data_only=True)
ws = db.active

# # Extract data
def get_title_data(workbook):
    """
    ...
    titleData = []
    for idx in range(8):
        titleData.append(str(workbook.cell(row=titleRow+idx, column=titleCol).value))
    
    # convert date to proper format
    # excel returns it as YYYY-MM-DD HH:MM:SS
    date = titleData[7]
    ...
    return (titleData, filename)

# Serviceablity Design Wind Pressure
def get_serviceability_data(workbook):
    """
...
def get_deflection_data(workbook):
    """
    ...
    posNameStart = (10, 7)
    negNameStart = (38, 7)
    posMembers = get_all_members(workbook, posNameStart)
    negMembers = get_all_members(workbook, negNameStart)
    deflectionVal = workbook.cell(row=14, column=3).value
    finalData = []
    ...
    for member in posMembers:
        name, row, col = member
        posRes = extract_deflection_data(workbook, (row, col))
        finalData.append((name, posRes, "placeholder"))
    
    for member in negMembers:
        name, row, col = member # get member details
        idx = check_name_included(finalData, name) # get index of the member in the array
        negRes = extract_deflection_data(workbook, (row, col))
        curName, posRes, placeholder = finalData[idx]
        finalData[idx] = (curName, posRes, negRes)
    

    return (finalData, deflectionVal)

def get_all_members(workbook, start):
    """
    ...
    while curRow <= 1048576:
        ...
        if default:
            break
        idx = check_name_included(arrayOfNames, currentName)
...
def check_name_included(array_of_names, value):
    """
...
def extract_deflection_data(workbook, coords):
    """
    ...
    return confirm_deflection_data(workbook, (curRow+8, curCol+4))

def confirm_deflection_data(workbook, coords):
    """
...
def get_operational_force(workbook):
    """
    ...
    while True:
        ...
        oFValue = (("{:.1f}".format(valOne), "{:.1f}".format(valTwo)), ("{:.1f}".format(valThree), "{:.1f}".format(valFour)))
        oFRow += 15
        oFSecondRow += 15

# Air infiltration
def get_aI_data(workbook):
    """
            ...
            if isinstance(waterValue, (int, float)):
                ...
                if isinstance(waterComments, str):
                    if isinstance(modVal, str):
                        value = (waterValue, waterComments, modVal)
                    else:
                        value = (waterValue, waterComments, "N/A")
                else:
                    value = (waterValue, "N/A", "N/A")
...
# UST Pressure
def get_ultimate_data(workbook):
    """
    ...
    while notDefault or fRow <= 1048576:
        ...
        if posTestVal not in ["", 0, None] and negTestVal not in ["", 0, None]:
            ultimateData = ("{:.0f}".format(posTestVal), "{:.0f}".format(negTestVal), obsvTable)
        else:
            return ultimateData
...
final = get_title_data(ws), get_deflection_data(ws), get_operational_force(ws), get_aI_data(ws), get_water_data(ws), get_ultimate_data(ws)
...
def check_with_gpt(array_of_values):
